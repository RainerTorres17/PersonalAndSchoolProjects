...
void * compute_image_threads(struct thread_args *args)
{
	...
	for(j=args->start;j<args->end;j++) {
		...
		for(i=0;i<width;i++) {
			...
			double y = args->ymin + j*(args->ymax-args->ymin)/bitmap_height(args->bmap);
...
int main( int argc, char *argv[] )
{
	...
	//splitting the work between the threads
	height = height / threads;

	//inputting values into a struct to pass a parameter in a thread
	...
	for(int i = 0; i < threads; i++){
		pthread_create(&tid[i],NULL, (void *)compute_image_threads, (void *)args);
		pthread_join(tid[i],NULL); 
	}

    //Accounting for input that doesnt split the threads evenly
	if(height % threads != 0){
		args->end = bitmap_height(bm);
